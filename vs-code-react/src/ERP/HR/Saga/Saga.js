...
//====================일근태관리 하인봉=================시작=====================일근태관리 하인봉======================================일근태관리 하인봉==================
function* dayAttd(action) {
  try {
    if (action.type === types.SEARCH_DAY_ATTD_LIST_REQUEST) {
      const { data } = yield call(() =>
        axios.get("http://localhost:8282/hr/attendance/dayAttendanceManage", {
          params: {
            cday: action.data.cday,
            name: action.data.name,
          },
        }),
      );
      yield put({ type: types.SEARCH_DAY_ATTD_LIST_SUCCESS, data: data });
...
export function* onManageATTD() {
  yield takeLatest(types.SEARCH_DAY_ATTD_LIST_REQUEST, dayAttd); //하인봉 근태관리
